JohnnieWalkerCoin-qt: Qt4 GUI for JohnnieWalkerCoin
===============================
...
Alternatively, install Qt Creator and open the `JohnnieWalkerCoin-qt.pro` file.

An executable named `JohnnieWalkerCoin-qt` will be built.
...
A warning for people using the *static binary* version of JohnnieWalkerCoin on a Linux/UNIX-ish system (tl;dr: **Berkely DB databases are not forward compatible**).

The static binary version of JohnnieWalkerCoin is linked against libdb5.3.
...
installed causes JohnnieWalkerCoin-qt to crash intermittently.  The issue has been reported as `launchpad bug 857790`_, but
isn't yet fixed.
