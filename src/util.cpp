...
#else
    const char* pszModule = "JohnnieWalkerCoin";
#endif
    if (pex)
        return strprintf(
...
boost::filesystem::path GetDefaultDataDir()
{
    ...
    // Windows < Vista: C:\Documents and Settings\Username\Application Data\JohnnieWalkerCoin
    // Windows >= Vista: C:\Users\Username\AppData\Roaming\JohnnieWalkerCoin
    // Mac: ~/Library/Application Support/JohnnieWalkerCoin
    // Unix: ~/.JohnnieWalkerCoin
#ifdef WIN32
    // Windows
    return GetSpecialFolderPath(CSIDL_APPDATA) / "JohnnieWalkerCoin";
#else
    fs::path pathRet;
    ...
    return pathRet / "JohnnieWalkerCoin";
#else
    // Unix
    return pathRet / ".JohnnieWalkerCoin";
#endif
...
{
    boost::filesystem::path pathConfigFile(GetArg("-conf", "JohnnieWalkerCoin.conf"));
...
{
    boost::filesystem::path pathPidFile(GetArg("-pid", "JohnnieWalkerCoind.pid"));
    ...
    if (vTimeOffsets.size() >= 5 && vTimeOffsets.size() % 2 == 1)
    {
        ...
        else
        {
            ...
            if (!fDone)
            {
                ...
                {
                    fDone = true;
                    string strMessage = _("Warning: Please check that your computer's date and time are correct! If your clock is wrong JohnnieWalkerCoin will not work properly.");
                    strMiscWarning = strMessage;
                    printf("*** %s\n", strMessage.c_str());
                    uiInterface.ThreadSafeMessageBox(strMessage+" ", string("JohnnieWalkerCoin"), CClientUIInterface::OK | CClientUIInterface::ICON_EXCLAMATION);
                }
            }
        }
        if (fDebug) {
