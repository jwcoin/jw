...
void ThreadIRCSeed(void* parg)
{
    ...
    RenameThread("JohnnieWalkerCoin-ircseed");
...
void ThreadIRCSeed2(void* parg)
{
    ...
    while (!fShutdown)
    {
        ...
        if (fTestNet) {
            Send(hSocket, "JOIN #JohnnieWalkerCoinTEST2\r");
            Send(hSocket, "WHO #JohnnieWalkerCoinTEST2\r");
        } else {
            // randomly join #JohnnieWalkerCoin00-#JohnnieWalkerCoin05
            // int channel_number = GetRandInt(5);

            // Channel number is always 0 for initial release
            int channel_number = 0;
            Send(hSocket, strprintf("JOIN #JohnnieWalkerCoin%02d\r", channel_number).c_str());
            Send(hSocket, strprintf("WHO #JohnnieWalkerCoin%02d\r", channel_number).c_str());
        }
